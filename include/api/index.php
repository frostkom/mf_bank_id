...
{
	case 'qr_init':
		$bankIDService = new BankIDService($api_url, apply_filters('get_current_visitor_ip', ""), $arr_params);
		...
		try
		{
			...
			switch($response->status)
			{
				...
				case 'complete':
					...
					$return_ssn = check_var('return_ssn');

					$user_ssn = $response->completionData->user->personalNumber;
					$user_ssn = $obj_bank_id->filter_ssn($user_ssn);

					$obj_bank_id->validate_and_login(array('action_type' => 'login', 'validation_type' => $validation_type, 'post_id' => $post_id, 'return_url' => $return_url, 'return_ssn' => $return_ssn, 'ssn' => $user_ssn), $json_output);
				break;
			...
			switch($response->status)
			{
				...
				case 'complete':
					...
					if($obj_bank_id->user_exists($user_ssn))
					{
						...
						$obj_bank_id->validate_and_login(array('action_type' => 'sign', 'validation_type' => $validation_type, 'post_id' => $post_id, 'return_url' => $return_url, 'return_ssn' => $return_ssn, 'ssn' => $user_ssn, 'response' => $response), $json_output);
